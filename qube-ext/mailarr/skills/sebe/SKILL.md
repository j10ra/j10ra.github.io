...
If an order requires config changes, use `routine_update` while the routine is unlocked. Sends work only after the user freezes the routine in the panel. Never ask for or attempt changes to the cap, session, enabled state, or frozen state.
