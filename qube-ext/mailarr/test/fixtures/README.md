...
Each fixture keeps one real representative record with its source-native field names and types. Long descriptions and feed metadata are shortened, and third-party email addresses use synthetic `.test` values, so parser tests stay focused and deterministic.
